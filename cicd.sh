...
runId=$(curl -v -k -L --header "Content-Type: application/json;charset=UTF-8" -X POST -d '{}' -u "${FX_USER}":"${FX_PWD}" "${FX_HOST}"/api/v1/runs/job/${FX_JOBID}?region=${REGION} | jq -r '.["data"]|.id')
...
#			echo  "Run detail link https://cloud.fxlabs.io"${array[6]}
			echo  "Run detail link "${FX_HOST}" Job-Run-No:"${array[6]}
			echo "------------------------------------------------"
			echo  "${array[7]}"
			echo "------------------------------------------------"
